...
use constant WEBWORK2_REPO => 'https://github.com/CCDMD/webwork2_fr_qc.git';
use constant PG_REPO       => 'https://github.com/CCDMD/pg_fr_qc.git';
use constant OPL_REPO =>
  'https://github.com/openwebwork/webwork-open-problem-library.git';
use constant BPL_REPO =>
  'https://github.com/CCDMD/webwork-banque-problemes-libres.git';
...
use constant WW_BRANCH => 'ccdmd-bdeb-H2017';
use constant PG_BRANCH => 'ccdmd-bdeb-H2017';
...
sub get_bpl_repo {
    ...
    my $repo    = $term->get_reply(
        #print_me => $print_me,
        prompt  => 'Where would you like to download the BPL from?',
        default => $default,
    );
    writelog($term->history_as_string()."\n");
    Term::UI::History->flush();
    ...
    $confirmed = confirm_answer($repo);
    if ($confirmed->{status}) {
        print_and_log("Got it, I'll download the BPL from $repo.\n");
        return $repo;
    } else {
        get_opl_repo($default);
    }
...
sub get_webwork {
    ...
    my $ww2_cmd = [$apps->{git},'clone',$ww2_repo,'webwork2'];

    my $pg_repo = get_pg_repo(PG_REPO);    #PG_REPO constant defined at top
    my $pg_cmd = [$apps->{git},'clone',$pg_repo,'pg'];

    my $opl_repo = get_opl_repo(OPL_REPO);    #OPL_REPO constant defined at top
    my $opl_cmd = [$apps->{git},'clone',$opl_repo];

    my $bpl_repo = get_bpl_repo(BPL_REPO);    #BPL_REPO constant defined at top
    my $bpl_cmd = [$apps->{git},'clone',$bpl_repo];
    ...
    my $bpl_success = run_command($bpl_cmd);;
    if ($opl_success) {
        print_and_log("Fetched BPL successfully\n");
    } else {
        print_and_log("Couldn't get BPL!\n");
    }
}
...
sub write_site_conf {
	...
	} elsif (/^\$siteDefaults\{locale\}/) {
	    print $out "\$siteDefaults\{locale\} = \"fr_CA.utf8\";\n";
        } else {
            print $out $_;
        }
    }
...
sub write_localOverrides_conf {
    ...
    while (<$in>) {
        if (/^\$language/) {
	    print $out "\$language = \"fr_CA\";\n";
        } elsif (/^\$problemLibrary\{root\}/) {
            print $out
"\$problemLibrary{root} = \"$WW_PREFIX/libraries/webwork-open-problem-library/OpenProblemLibrary\";\n";
	} elsif (/^\$contribLibrary\{root\}/) {
	    print $out
"\$contribLibrary{root} = \"$WW_PREFIX/libraries/webwork-open-problem-library/Contrib\";\n";
        } elsif (/^\$pg\{options\}\{displayMode\}/) {
            print $out "\$pg{options}{displayMode} = \"MathJax\";\n";
	} elsif (/^\$courseFiles\{problibs\}/) {
	    print $out "\$courseFiles\{problibs\}  = \{\n";
	    print $out "         BPL => \"Banque de problèmes libres\",\n";
	} elsif (/^\# \$studentDateDisplayFormat/) {
	    print $out "\$char=chr(224);\n";
	    print $out "\$studentDateDisplayFormat=\"\%a \%d \%b \%Y \$char \%l:\%M\%P\";\n";  
	} elsif(/^1;/) {
	    print $out "\# Other configurations of the CCDMD that override default.config\n\n";
	    print $out "\$webworkURLs{bugReporter} = \"https://goo.gl/forms/nxnjd8K1DhtQiIU02\";\n\n";
	    print $out "\$pg{options}{mathViewLocale} = \"mv_locale_fr.js\";\n\n";
 	    print $out "\$mail{feedbackSubjectFormat} = \"[WWfeedback] Cours : \%c, Utilisateur : \%u, Devoir : \%s,  Probl. : \%p\";\n\n";
 	    print $out "\$pg{specialPGEnvironmentVars}{MathView} = 1;\n\n";
	    print $out "1; \#final line of the file to reassure perl that it was read properly.\n";
	} else {
            print $out $_;
        }
    }
...
sub setup_bpl {
    my $WW_PREFIX = shift;
    symlink(
        "$WW_PREFIX/libraries/webwork-banque-problemes-libres/BPL",
        "$WW_PREFIX/courses/modelCourse/templates/Library"
    );
    #system("$WW_PREFIX/webwork2/bin/BPL-update");
